...
\begin{itemize}
  ...
  \item \texttt{DELETE /api/datasets/:id} — removes a dataset from the
    registry.
  \item \texttt{POST /api/helmert} — applies a seven-parameter Helmert
    transformation computed from ground control points, returns the derived
    model matrix.
  ...
  \item \texttt{GET /cesium-proxy/*} — transparently proxies the CesiumJS
    library and Web Workers from the CDN, patching the source to remove
    float-precision artefacts introduced by the proprietary Cesium ion
    coordinate path, and to fix the polar orbit constraint on the
    \texttt{ScreenSpaceCameraController}.
\end{itemize}
...
\texttt{type}       & enum      & \texttt{cesium | cesium-splat | splat | e57 | pointcloud} \\
...
\end{equation}
where $(E_0, N_0, H_0)$ is the dataset centroid converted to WGS\,84 via the
approximate LV95\,$\to$\,WGS\,84 formula implemented in
\texttt{lv95ToWgs84()}, $\mathbf{M}_{\text{ENU}}$ is the
...
\begin{description}
  \item[Point clouds (LAS/LAZ, E57, XYZ/TXT/PTS, PTX, PCD, PLY)] — converted
    ...
    \texttt{tileset.json}. XYZ colour attributes are preserved; the dataset
    centroid is used as the \texttt{RTC\_CENTER} for float32 precision. A
    LV95 model matrix is computed and stored.
  \item[Meshes (OBJ, STL, GLB, GLTF, mesh PLY)] — converted to a
    \texttt{.b3dm}-based 3D Tiles scene.
  \item[Gaussian splats (.splat, 3DGS PLY)] — the \texttt{.splat} binary is
    served directly to the \texttt{@mkkellogg/gaussian-splats-3d} viewer;
    a companion 3D Tiles representation is generated from the DC spherical
...
tileset-local) and enters the known LV95 coordinates. A seven-parameter Helmert
transformation (rotation, uniform scale, translation) is computed via SVD of
the centred point pairs, yielding the $4 \times 4$ model matrix
$\mathbf{M} = \mathbf{M}_{\text{LV95}} \cdot \mathbf{M}_{\text{Helmert}}$
that is stored persistently in the dataset registry.
